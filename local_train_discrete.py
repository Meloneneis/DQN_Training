...
def main():
    """Train discrete action model with fiery teacher"""

    print("python version:\t{0}".format(platform.python_version()))
    print("gym version:\t{0}".format(gym.__version__))

    # Hyperparameters
    action_config = 'xlarge_full'  # 16 actions
    action_repeat = 2
    activation = 'gelu'
    batch_size = 32
    buffer_size = 200000
    ...
    exploration_fraction = 0.1987343116573252
    gamma = 0.999
    ...
    weight_decay = 0.00003514293012621225
    ...
    # Fixed parameters
    agent_name = 'discrete_fullaction'
    outdir = 'local_models'
    no_display = True
    use_continuous_actions = False

    # Warm-up configuration - Fiery teacher
    warmup_teacher_path = 'fiery.pth'
    warmup_steps = 60000
    warmup_teacher_arch = {
    ...
    wandb.init(
        ...
        config={
            ...
            'training_type': 'discrete_fullaction_with_fiery_warmup',
        },
        tags=['local-training', 'fiery-teacher', 'discrete-fullaction', 'large_5layer', 'large_2layer', 'gelu', '60k-warmup', '50-validation-seeds']
    )
    ...
    # Print configuration
    print("\n=== Discrete Full Action Training Configuration ===")
    ...
    print()
    print(f"🔥 WARM-UP MODE: Using fiery teacher for first {warmup_steps} steps")
    ...
    # Start training
    print("\nStarting discrete full action training with warm-up...\n")
    deepq.learn(
        env,
        lr=lr,
        total_timesteps=total_timesteps,
    ...
    print(f"\n✓ Discrete full action training complete! Model saved in {outdir}/")

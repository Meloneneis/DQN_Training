...
def main():
    """Train Tokyo Drift model with chocolate teacher"""
    ...
    batch_size = 32
    buffer_size = 200000
    cnn_config = 'xlarge_4layer'  # Largest CNN (5 layers)
    dropout_rate = 0
    early_stopping_patience = 5
    exploration_final_eps = 0.035
    exploration_fraction = 0.22087886429795167
    gamma = 0.999
    hidden_layer_config = 'large_2layer'  # Large 2-layer
    learning_starts = 500
    lr = 0.0001
    lr_scheduler = 'cosine'
    normalization = 'layer'
    ...
    weight_decay = 0.00002084006397885873
    ...
    # Fixed parameters
    agent_name = 'tokyo_drift'
    outdir = 'local_models'
    no_display = True
    use_continuous_actions = False

    # Warm-up configuration - Chocolate teacher
    warmup_teacher_path = 'chocolate.pth'
    warmup_steps = 60000
    warmup_teacher_arch = {
    ...
    wandb_run_name = f"tokyo-drift-chocolate-teacher-large5-large2-gelu-60k-warmup"
    ...
    wandb.init(
        ...
        config={
            ...
            'training_type': 'tokyo_drift_with_chocolate_warmup',
        },
        tags=['local-training', 'chocolate-teacher', 'tokyo-drift', 'large_5layer', 'large_2layer', 'gelu', '60k-warmup', '50-validation-seeds']
    )
    ...
    # Print configuration
    print("\n=== Tokyo Drift Training Configuration ===")
    ...
    print()
    print(f"🔥 WARM-UP MODE: Using chocolate teacher for first {warmup_steps} steps")
    ...
    # Start training
    print("\nStarting Tokyo Drift training with warm-up...\n")
    deepq.learn(
        env,
        lr=lr,
        total_timesteps=total_timesteps,
    ...
    print(f"\n✓ Tokyo Drift training complete! Model saved in {outdir}/")

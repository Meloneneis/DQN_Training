...
def main():
    """Train with pretty as teacher"""

    print("python version:\t{0}".format(platform.python_version()))
    print("gym version:\t{0}".format(gym.__version__))

    # Hyperparameters (same as other students)
    action_config = 'large_precision'  # 12 actions (pretty's action set)
    action_repeat = 2
    activation = 'gelu'
    batch_size = 32
    buffer_size = 200000
    cnn_config = 'large_5layer'
    dropout_rate = 0
    early_stopping_patience = 5
    exploration_final_eps = 0.035
    exploration_fraction = 0.2465571373219292
    gamma = 0.999
    ...
    weight_decay = 0.0004484914808534276
    ...
    # Fixed parameters
    agent_name = 'pretty_student'
    outdir = 'local_models'
    no_display = True
    use_continuous_actions = False

    # Warm-up configuration - Pretty teacher
    warmup_teacher_path = 'pretty.pth'
    warmup_steps = 60000
    warmup_teacher_arch = {
    ...
    wandb_run_name = f"pretty-student-pretty-teacher-large5-large2-gelu-60k-warmup"
    ...
    wandb.init(
        ...
        config={
            ...
            'training_type': 'pretty_student_with_pretty_warmup',
        },
        tags=['local-training', 'pretty-teacher', 'large_precision', 'large_5layer', 'large_2layer', 'gelu', '60k-warmup', '50-validation-seeds']
    )
    ...
    # Print configuration
    print("\n=== Pretty Student Training Configuration ===")
    ...
    print()
    print(f"🔥 WARM-UP MODE: Using pretty teacher for first {warmup_steps} steps")
    ...
    # Start training
    print("\nStarting pretty student training with warm-up...\n")
    deepq.learn(
        env,
        lr=lr,
        total_timesteps=total_timesteps,
    ...
    print(f"\n✓ Pretty student training complete! Model saved in {outdir}/")
